...
void TypeChecker::visitNode(BindingNode* node) { 
    ...
    if (node->type == "Inferred") {
        if (node->value->type == "Void") {
            ...
            throw std::runtime_error("Type check failed for ShuLang AST Node");
        }
...
void TypeChecker::visitNode(NotNode* node) { 
    descendIntoChildren(node);
    assert_same("Boolean", node->value->type, "\'not\' had the wrong type!");
}

void TypeChecker::visitNode(BeginNode* node) {
    descendIntoChildren(node);
    node->type = node->end_value->type;
}

void TypeChecker::visitNode(SelectOperatorNode* node) {
    descendIntoChildren(node);
